...
exports.VIEWPORT_PRESETS = void 0;
exports.closeBrowser = closeBrowser;
exports.renderUrl = renderUrl;
exports.renderHtml = renderHtml;
const puppeteer_1 = __importDefault(require("puppeteer"));
const dom_walker_js_1 = require("./dom-walker.js");
...
function resolveViewport(option) {
    if (!option)
        return exports.VIEWPORT_PRESETS.desktop;
    if (typeof option === 'string') {
        return exports.VIEWPORT_PRESETS[option] || exports.VIEWPORT_PRESETS.desktop;
    }
    return option;
}
...
async function getBrowser() {
    ...
    browserLaunching = puppeteer_1.default.launch({
        headless: true,
        args: [
            '--no-sandbox',
...
async function forceLazyImages(page) {
    await page.evaluate(() => {
        ...
        document.querySelectorAll('img[data-src], img[data-lazy-src], img[data-original]').forEach((img) => {
            const imgEl = img;
            const lazySrc = imgEl.getAttribute('data-src') ||
...
async function extractDom(page) {
    const script = (0, dom_walker_js_1.getDomWalkerScript)();
 ...
 */
async function renderUrl(url, viewportOption) {
 ...
 */
async function renderHtml(html, css, viewportOption) {
    const viewport = resolveViewport(viewportOption);
    const page = await setupPage(viewport);
    try {
