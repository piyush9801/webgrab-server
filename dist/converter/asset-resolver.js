...
exports.extractBestImageUrl = extractBestImageUrl;
exports.proxyAssetUrl = proxyAssetUrl;
exports.proxyAllAssets = proxyAllAssets;
 ...
 */
function extractBestImageUrl(attributes) {
 ...
 */
function proxyAssetUrl(originalUrl, serverBase) {
    if (!originalUrl)
        return '';
    ...
    if (originalUrl.startsWith(serverBase))
        return originalUrl;
 ...
 */
function proxyAllAssets(node, baseUrl, serverBase) {
    if (!node)
        return;
    // Proxy image src
    ...
    if (node.styles?.backgroundImage) {
        ...
        if (urlMatch && urlMatch[1]) {
            ...
            node.styles.backgroundImage = bgImage.replace(urlMatch[0], `url("${resolved}")`);
        }
    }
