 ...
 */
export function parseBoxShadow(shadowStr: string): ParsedShadow[] {
...
function parseSingleShadow(str: string): ParsedShadow | null {
  let inset = false;
  let working = str;
  ...
  let color: RGBAColor = { r: 0, g: 0, b: 0, a: 1 }; // default black
 ...
 */
export function parseTransform(transformStr: string): ParsedTransform {
  ...
  if (matrixMatch) {
    ...
    result.scaleY = Math.sqrt(
      parseFloat(matrixMatch[3]) ** 2 + parseFloat(matrixMatch[4]) ** 2
    );
 ...
 */
export function parseLetterSpacing(spacing: string): number {
