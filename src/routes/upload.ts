...
async function handleHtmlFile(buffer: Buffer): Promise<string> {
  return buffer.toString('utf-8');
}

async function handleZipFile(buffer: Buffer): Promise<string> {
  const zip = new AdmZip(buffer);
  const entries = zip.getEntries();

  // Look for index.html first
  let htmlEntry = entries.find(
    (e) => e.entryName.toLowerCase() === 'index.html' || e.entryName.toLowerCase().endsWith('/index.html')
  );
...
async function handleEmailFile(buffer: Buffer, ext: string): Promise<string> {
  try {
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      console.log(
        `[upload] Processing file: ${file.originalname} (${(file.size / 1024).toFixed(1)}KB, type: ${ext})`
      );
      ...
      const vpParam = (req.body?.viewport || req.query?.viewport) as string | undefined;
      if (vpParam && ['desktop', 'tablet', 'mobile'].includes(vpParam)) {
        viewport = vpParam as 'desktop' | 'tablet' | 'mobile';
      }

      console.log(`[upload] Rendering extracted HTML (${html.length} chars)`);

      // Render the HTML
      const renderResult = await renderHtml(html, undefined, viewport);
